...
//         const { data: currentUser } = await axios.get("http://localhost:8082/api/auth/profile", { headers });
//         setUser(currentUser);
//         if (!currentUser?._id) throw new Error("Invalid user data received");

//         const { data: reviewsResponse } = await axios.get(
//           `http://localhost:8082/api/review/user/${currentUser._id}`,
//           { headers }
//         );
...
//       const token = localStorage.getItem("token");
//       const headers = { Authorization: `Bearer ${token}` };
//       await axios.delete(`http://localhost:8082/api/review/delete/${reviewId}`, { headers });
...
// //         } else {
// //           formData.append(`existingDocs[${index}]`, JSON.stringify(doc));
// //         }
// //       });

// //       const { data } = await axios.put(
// //         `http://localhost:8082/api/review/update/${reviewId}`,
// //         formData,
// //         { headers: { ...headers, "Content-Type": "multipart/form-data" } }
// //       );
...
// //       if (updatedProfileImage) formData.append("profileImage", updatedProfileImage);

// //       const { data } = await axios.put(
// //         "http://localhost:8082/api/auth/update-profile",
// //         formData,
// //         {
// //           headers: {
// //             ...headers,
...
// //       const token = localStorage.getItem("token");
// //       const headers = { Authorization: `Bearer ${token}` };
// //       await axios.delete("http://localhost:8082/api/auth/delete-profile", { headers });
...
const handleDeleteProfile = async () => {
  if (!window.confirm("Are you sure you want to permanently delete your profile and all data?")) {
    return;
  }
  ...
  try {
    ...
    // Get the current user ID (you might need to adjust this based on your user state)
    const userId = user?._id; // Make sure your user state has the _id
    ...
    alert(
      ...
      "Failed to delete profile. Please check with the administrator."
    );
  }
};
